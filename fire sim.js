      ...
      points.push(
        new addpoint(
          ...
          (j * gridh * fillin) / (amount - 1) +
            miny +
            (gridh * (1 - fillin)) / 2,
          random(3000, 3000) * windowscale ** 2
        )
      );
...
const externalForces = function () {
  points.forEach((data) => {
    ...
    if (Math.abs(data.x - centerx) < 600) {
      data.temperature += (max(100 - (gridh - data.y), 0) / 100) ** 2 * 10;
    }
    data.vy -= data.temperature / 800;
  });
};
...
const gettemp = function (data1, index) {
  let temp = 0;
  if (index < 0 || index > gridx * gridy - 1) return 0;
...
const newgetdensity = function (point) {
  let i =
  ...
  for (let j = -1; j < 2; j++) {
    for (let k = -1; k < 2; k++) {
      temp += gettemp(point, i + j + gridx * k);
    }
  }
...
const applyforce = function (point1, point2, i1, i2) {
  ...
  if (distance < smoothingradius) {
    ...
    let sharedpressure =
      ((pointdensities[i1] +
        point1.temperature / 400 +
        pointdensities[i2] +
        point2.temperature / 400) /
        2 -
        targetdensity) *
      multiplier;
    ...
    return [
      ...
      ((point2.temperature - point1.temperature) *
        ...
        point2.mass) /
        pointdensities[i2],
      ((point1.temperature - point2.temperature) *
        influence *
        tempscale *
        point2.mass) /
        pointdensities[i2],
...
const gridforce = function (bound) {
  bound.forEach((id, i) => {
    for (let i2 = i + 1; i2 < bound.length; i2++) {
      ...
      if (returned != 0) {
        let [vx1, vy1, vx2, vy2, t1, t2] = returned;
        points[id].vx += vx1;
        points[id].vy += vy1;
        points[id].temperature += t1;
        points[n].vx += vx2;
        points[n].vy += vy2;
        points[n].temperature += t2;
      }
    }
  });
};
  ...
  i.forEach((id1) => {
    grid[i2].forEach((id2) => {
      ...
      if (returned != 0) {
        ...
        points[id1].vx += vx1;
        points[id1].vy += vy1;
        points[id1].temperature += t1;
        points[id2].vx += vx2;
        points[id2].vy += vy2;
        points[id2].temperature += t2;
      }
    });
  });
};
  ...
  if (rendermode == 1) {
    points.forEach((data, i) => {
      if (data.temperature > 0) {
        ...
        c = data.temperature;
        c = 1 - max(1 - data.temperature / 300, 0) ** 1.4;
        if (c > 0.1) {
          strokeWeight(c * 10);
          if (c > 0.8) {
            c = (c - 0.8) / 0.2;
            stroke(7 + c * 5, 65 + c * -35, 100);
          } else {
            c = c / 0.8;
            stroke(5 + c * 2, 100 - c * 35, 100);
          }
...
const visualise = function () {
  ...
  for (let x = minx + resolution; x < minx + gridw; x += floor(resolution)) {
    for (let y = miny + resolution; y < miny + gridh; y += floor(resolution)) {
      let c = newgetdensity({ x: x, y: y });

      c = 1 - max(1 - c / 300, 0) ** 1.4;
